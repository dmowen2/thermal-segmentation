...
class UNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.encoder = models.densenet169(weights=models.DenseNet169_Weights.IMAGENET1K_V1)

        # Extract the feature extraction layers from DenseNet201
        self.encoder_layers = nn.Sequential(
            self.encoder.features,  # The entire DenseNet feature extractor
        )

        # Decoder layers
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(1664, 832, kernel_size=2, stride=2),  # Adjusted for DenseNet feature sizes
            nn.ReLU(),
            nn.Conv2d(832, 416, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(416, 208, kernel_size=2, stride=2),
            nn.ReLU(),
            nn.Conv2d(208, 104, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(104, 64, kernel_size=2, stride=2),
            nn.ReLU(),
...
weights_path = "best_weights_densenet169.pth"  # Ensure you have the correct weights file
...
test_new_images(model, new_image_paths, ground_truth_paths, save_folder='densenetimg/densenet169')
