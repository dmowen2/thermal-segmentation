...
class UNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.encoder = timm.create_model('resnext50_32x4d', pretrained=True, features_only=True, out_indices=(1, 2, 3, 4))
        
        # Extract the feature extraction layers from ResNeXt-50
        self.encoder_layers = lambda x: self.encoder(x)[-1] if isinstance(self.encoder(x), list) else self.encoder(x)

        # Decoder layers (adjusted for ResNeXt-50's final feature map size)
...
weights_path = "best_weights_resnext50.pth"  # Ensure you have the correct weights file
...
test_new_images(model, new_image_paths, ground_truth_paths, save_folder='resnextimg/resnext50')
