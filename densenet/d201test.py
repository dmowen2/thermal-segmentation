...
class UNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.encoder = models.densenet201(weights=models.DenseNet201_Weights.IMAGENET1K_V1)

        # Extract feature extraction layers from DenseNet201
        self.encoder_layers = nn.Sequential(
            self.encoder.features,  # Full feature extractor from DenseNet201
        )

        # Decoder layers (Adjusted for DenseNet201 output size)
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(1920, 960, kernel_size=2, stride=2),  # Adjusted input channels
            nn.ReLU(),
            nn.Conv2d(960, 480, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(480, 240, kernel_size=2, stride=2),
            nn.ReLU(),
            nn.Conv2d(240, 120, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(120, 64, kernel_size=2, stride=2),
            nn.ReLU(),
...
weights_path = "best_weights_densenet201.pth"  # Ensure you have the correct weights file
...
test_new_images(model, new_image_paths, ground_truth_paths, save_folder='densenetimg/densenet201')
