...
class UNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.encoder = timm.create_model('tf_efficientnet_b4', pretrained=True, features_only=True, out_indices=(0, 1, 2, 3, 4))
        
        # Extract the feature extraction layers from InceptionV3
        self.encoder_layers = lambda x: self.encoder(x)[-1] if isinstance(self.encoder(x), list) else self.encoder(x)
        
        # Decoder layers
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(448, 256, kernel_size=2, stride=2),  # Adjusted input channels
            nn.ReLU(),
...
def test_new_images(model, image_paths, ground_truth_paths=None, img_size=(256, 256), save_folder='seresnetimg/seresnet152'):
    model.to(device)
    model.eval()
...
weights_path = "best_weights_effnetb4.pth"  # Ensure you have the correct weights file
...
test_new_images(model, new_image_paths, ground_truth_paths, save_folder='effnetimg/effnetb4')
